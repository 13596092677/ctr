...
void print_usage_and_exit() {
  ...
  printf("*********************************collaborative topic models for recommendations with social matrix factorization************************\n");
  printf("Authors: Chong Wang, chongw@cs.princeton.edu, Computer Science Department, Princeton University,\n");
  printf("         Kyrre Wahl Kongsgaard, kyrre@science.uva.nl, Computer Science Department, University of Amsterdam.\n");
  ...
  printf("      --followers:      follower file, optional\n");
  ...
  printf("      --lambda_q:       adjacency vector regularizer, default 100\n");
  printf("      --lambda_s:       social factor regularizer, default 100\n");
  printf("\n");
...
int main(int argc, char* argv[]) {
  ...
  const char* const short_options = "hd:x:i:a:b:u:v:r:s:m:k:t:e:y:f";
  ...
  if (follower_path != NULL) {
    if (!file_exists(follower_path)) {
        printf("follower file %s doesn't exist! quit ...\n", follower_path);
        exit(-1);
    }
    printf("follower file: %s\n", follower_path);
  }
  ...
  ctr_param.set(a, b, lambda_u, lambda_v, lambda_q, random_seed, max_iter, save_lag, theta_opt, ctr_run);
  ...
  c_data* followers = NULL;

  if (follower_path != NULL) {
    // read followers
    printf("reading adjacency matrix from %s ...\n", follower_path);
    followers = new c_data(); 
    followers->read_data(follower_path);
    int num_items = (int)followers->m_vec_data.size();
  }
  ...
  ctr->learn_map_estimate_social(users, items, followers, c, &ctr_param, 
                                 directory);
