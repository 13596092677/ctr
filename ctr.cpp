...
void c_ctr::learn_map_estimate_social(const c_data* users, const c_data* items,
  ...
  while ((iter < param->max_iter and converge > 1e-6 ) or iter < min_iter) {
    ...
    //gsl_blas_sgemm (CblasNoTrans, CblasTrans, 1., m_S, m_S, 0., S);
    for (j = 0; j < m_num_users; j ++) {
      m = followers->m_vec_len[j];
      if (m>0) {
        gsl_vector_const_view v = gsl_matrix_const_row(m_S, j); 
        gsl_blas_dger(1.0, &v.vector, &v.vector, S);
      }

    }
    gsl_matrix_scale(S, param->b * param->lambda_q);
